...
if __name__ == '__main__':
    ...
    print('Now the admin includes this user into its trusted list.')
    admin.append_users(user)

    message1 = Message(admin, '{}/cmd'.format(device1.deviceid), 'Start')
    message2 = Message(admin, '{}/cmd'.format(device2.deviceid), 'Close')
    message3 = Message(user, '{}/cmd'.format(device1.deviceid), 'turn temperature to 72')
    message4 = Message(user, '{}/cmd'.format(device2.deviceid), 'Start when the time is 9:00 a.m.')
    print('Admin starts to send message to both devices')
    print('Admin ------Start------> 1/cmd')
    print('Admin ------Close------> 2/cmd')
    print('User starts to send message to both devices')
    print('User ------turn temperature to 72------> 1/cmd')
    print('User ------Start when the time is 9:00 a.m.------> 2/cmd')
    admin.publish(message1, cloud)
    admin.publish(message2, cloud)
    user.publish(message3, cloud)
    user.publish(message4, cloud)
